...
<xsl:template match="fig | table-wrap">
  ...
  <xsl:variable name="slideshowURL">
    <xsl:value-of select="concat('/article/slideshow.action?uri=',substring($imageURI, 1, (string-length($imageURI)-5)),'&amp;imageURI=',$imageURI)"/>
  </xsl:variable>
  ...
  <div class="figure">
    ...
    <xsl:element name="a">
      ...
      <xsl:element name="img">
        ...
        <xsl:attribute name="src"><xsl:value-of select="concat('/article/fetchObject.action?uri=',$imageURI,'&amp;representation=PNG_S')"/></xsl:attribute>
    ...
    <xsl:if test="../../back/glossary">
      <p>
        ...
        <xsl:for-each select="../../back/glossary/def-list/def-item">
          <xsl:value-of select="term"/>, <xsl:value-of select="def/p"/><xsl:if test="position() != last()">; </xsl:if>
        </xsl:for-each>
      </p>
    </xsl:if>
<!--  <p>
...
<xsl:template match="supplementary-material">
 ...
 <h5 xpathLocation="noSelect">
   <xsl:element name="a">
     ...
     <xsl:attribute name="href"><xsl:value-of select="concat('/article/fetchFirstRepresentation.action?uri=',$objURI)"/></xsl:attribute>
...
<xsl:template match="inline-graphic">
  <xsl:element name="img">
    ...
    <xsl:attribute name="src"><xsl:value-of select="concat('/article/fetchObject.action?uri=',$graphicDOI,'&amp;representation=PNG')"/></xsl:attribute>
...
<!--    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:if test="citation[@citation-type='journal']">
          ...
          <xsl:variable name="findURL"><xsl:value-of select="concat('/article/findArticle.action?author=', $author, '&amp;title=', $artTitle)"/></xsl:variable>
