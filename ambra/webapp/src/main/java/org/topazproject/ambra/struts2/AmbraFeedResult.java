...
public class AmbraFeedResult extends Feed implements Result {
  ...
  public void execute(ActionInvocation ai) throws Exception {
    ...
    switch (cacheKey.feed_type()) {
      case Annotation :
        entries = buildAnnotationFeed(cacheKey, xmlBase);
        break;
      case Article :
   ...
   */
  private List<Entry> buildAnnotationFeed (Key cacheKey, String xmlBase) throws Exception {
    // Add each Article as a Feed Entry
  ...
  private List<Content>newAnnotationsList (WebAnnotation annotation, Link link)
    ...
    text.append("<p>");
    if (displayName != null)
      text.append(displayName + " on ");

    text.append(" <a href=" + link.getHref() + ">" + link.getTitle() + "</a></p>");
    text.append("<p>" + annotation.getComment() + "</p>");
    description.setValue(text.toString());
    contents.add(description);
  ...
  /**
   * Set the articleFeedService.
   *
   * @param  articleFeedService
   */
