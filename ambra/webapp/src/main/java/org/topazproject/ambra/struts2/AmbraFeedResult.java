 ...
 *                              Set via the ValueStack <code>ai.getStack().findValue("articleIDs")</code> call
 *
 * ArticleFeed.Key cacheKey     The cache key with input parameters of the request. Some of which affect the
 *                              format of the output. Set via  <code>ai.getStack().findValue("cacheKey")</code> call
 *
 * </pre>
 * <h4>TODO:</h4>
 * <pre>
 ...
 * ####  2008/10/10 (WTO)             Yanked the feed construction out of the action object and placed it
 *                                    in a result. I also re-organized and broke up the extremely long
 *                                    execute method that does the actually work. Hopefully in a more
 *                                    comprehensible fashion.
 * </pre>
 *
 * @see       org.topazproject.ambra.article.action.ArticleFeed.Key
 * @see       ArticleFeed
 *
 */
public class AmbraFeedResult extends Feed implements Result {
 ...
 //TODO: WTO Need to put these in a resource bundle.
  ...
  private static final String  COPYRIGHT_DEF = "This work is licensed under a Creative Commons Attribution-Share Alike 3.0 License, http://creativecommons.org/licenses/by-sa/3.0/";

  //A bunch of helper methods which were defined inline but I cannot stand the clutter.
  private       String JRNL_URI()     { String URI = CONF.getString("ambra.virtualJournals." + getCurrentJournal() + ".url",CONF.getString("ambra.platform.webserver-url", URL_DEF));
                                        return (URI.endsWith("/")) ? URI: URI + "/"; }
  private String JOURNAL_NAME() { return journalConfGetString("ambra.platform.name", PLATFORM_NAME_DEF); }
  private String FEED_TITLE()   { return journalConfGetString("ambra.services.feed.title", FEED_TITLE_DEF); }
  private String FEED_TAGLINE() { return journalConfGetString("ambra.services.feed.tagline", TAGLINE_DEF); }
  private String FEED_ICON()    { return journalConfGetString("ambra.services.feed.icon", JRNL_URI() + ICON_DEF); }
  private String FEED_ID()      { return journalConfGetString("ambra.services.feed.id", FEED_ID_DEF); }
  private String FEED_EXTENDED_NS()     { return journalConfGetString("ambra.services.feed.extended.namespace", FEED_NS_DEF); }
  private String FEED_EXTENDED_PREFIX() { return journalConfGetString("ambra.services.feed.extended.prefix", PREFIX_DEF); }
  private String JOURNAL_EMAIL_GENERAL(){ return journalConfGetString("ambra.platform.email.general", EMAIL_DEF); }
  private String JOURNAL_COPYRIGHT()    { return  journalConfGetString("ambra.platform.copyright", COPYRIGHT_DEF);}

 /**
  *  AmbraFeedResult constructor. Creates a atom_1.0 wire feed with UTF-8 encoding and Xmlbase
  *  based on the journal URI.
  */
  ...
  public void execute(ActionInvocation ai) throws Exception {
   ...
   final URI uri = (pathInfo == null) ? URI.create("/") : URI.create(pathInfo);
    ...
    List<String> IDs = (List<String>) ai.getStack().findValue("articleIDs");
    ArticleFeed.Key cacheKey = (ArticleFeed.Key)ai.getStack().findValue("cacheKey");

    //Fetch the article with the ID's generated by FeedAction
    articles = articleOtmService.getArticles(IDs);
    ...
    for (Article article : articles) {
      ...
      Entry entry = newEntry(article);  // Create an entry for the article.

      List<Link> altLinks = new ArrayList<Link>();

      //Linkt to article via xmlbase
   ...
   */
  private List<Content> newContentsList(ArticleFeed.Key cacheKey, Article article, String authorNames, int numAuthors) {
      ...
      if (bc != null) {
        ...
        if (cacheKey.isExtended()) {
           ...
           for (UserProfile profile: authorProfiles) {
             ...
             // Not extended therefore there will only
             // be one author.
             Person person = new Person();
  ...
  private Link newAltLink (Article article, Representation rep, String xmlBase) {
    ...
    altLink.setHref(xmlBase + fetchObjectAttachmentAction + "?uri=" + dc.getIdentifier() + "&representation=" + rep.getName());
  ...
  private Link newSelfLink (Article article, String xmlBase) {
    ...
    try {
        link.setHref(xmlBase + "article/" + URLEncoder.encode(dc.getIdentifier(), "UTF-8"));
      } catch(UnsupportedEncodingException uee) {
        link.setHref(xmlBase + "article/" + dc.getIdentifier());
  ...
  private void output() throws IOException {
    ...
    try {
      ...
      throw new RuntimeException(e);
       ...
       // WTO: respOut.output throws an
       // exception if respStrm is null
       respStrm.close();
    }
 }
  /**
   ...
   */
  private String journalConfGetString( String key, String defaultValue) {
