...
public class AccessClientFactory extends AbstractReAuthStubFactory {
  ...
  /**
   * Creates a Access service client stub..
   *
   * @param service the service configuration (url and credentials)
   *
   * @return Returns an Access service client stub.
   *
   ...
   */
  public static Access create(ProtectedService service)
                             throws MalformedURLException, ServiceException {
    Access stub = instance.createStub(service);

    if (service.hasRenewableCredentials())
      stub = (Access) instance.newProxyStub(stub, service);

    return stub;
  }

  private Access createStub(ProtectedService service)
                           throws MalformedURLException, ServiceException {
    URL                        url     = new URL(service.getServiceUri());
    AccessServiceLocator locator = new AccessServiceLocator();

    locator.setMaintainSession(true);

    Access client = locator.getAccessServicePort(url);
   ...
   *
   * @param serviceUri the uri for Access service
   *
   * @return Returns a Access service client stub.
   *
   * @throws MalformedURLException If the service url is misconfigured
   * @throws ServiceException If there is an error in creating the stub
   */
  public static Access create(String serviceUri)
                             throws MalformedURLException, ServiceException {
